...
public class Elevator extends SubsystemBase implements Constants.Elevator {
    ...
    public void setPosition() {
        ...
        elbowSetpoint = new TrapezoidProfile.State(elbowMotor.getPosition().getValue() / elbowTicksPerRadian, 0);
        wristSetpoint = new TrapezoidProfile.State(wristMotor.getPosition().getValue()  / wristTicksPerRadian, 0);
    }
    /**
    ...
    *@param wristPos joint position for wrist
    ...
    public void setArmPosition(double elbowPos, double wristPos){
        elbowMotor.set(TalonFXControlMode.Position,
            ...
            elbowKG * Math.cos(elbowMotor.getPosition().getValue()  / (elbowTicksPerRadian)));

        wristMotor.set(TalonFXControlMode.Position,
            wristPos * wristTicksPerRadian, DemandType.ArbitraryFeedForward,
            wristKG * Math.cos(wristMotor.getPosition().getValue()  / (wristTicksPerRadian)));
    }
    ...
    public void periodic() {
        SmartDashboard.putNumber("elbow/elbow position", elbowMotor.getPosition().getValue() );
        SmartDashboard.putNumber("elbow/elbow voltage", elbowMotor.getMotorVoltage().getValueAsDouble());
        SmartDashboard.putNumber("elbow/elbow position degrees", elbowMotor.getPosition().getValue()  / elbowTicksPerDegree);
        SmartDashboard.putNumber("elbow/elbow velocity", getElbowVelocity());

        SmartDashboard.putNumber("wrist/wrist position", wristMotor.getPosition().getValue() );
        SmartDashboard.putNumber("wrist/wrist voltage", wristMotor.getMotorVoltage().getValueAsDouble());
        SmartDashboard.putNumber("wrist/wrist position degrees", wristMotor.getPosition().getValue()  / wristTicksPerDegree);
    ...
    * @return Returns whether the variable coneOrCube is true or false
    ...
    public double getElbowPosition() {
        return elbowMotor.getPosition().getValue() / elbowTicksPerRadian;
    }

    public double getElbowVelocity() {
        return elbowMotor.getVelocity().getValue()  / elbowTicksPerRadian / 10;
    }
    ...
    public double getWristPosition() {
        return wristMotor.getPosition().getValue()  / wristTicksPerRadian;
    }

    public double getWristVelocity() {
        return wristMotor.getVelocity().getValue()  / elbowTicksPerRadian / 10;
    }

    public void setArmCoast(){
        elbowMotor.setNeutralMode(NeutralModeValue.Coast);
